...
class ProductProduct(models.Model):
    ...
    def _compute_last_purchase_line_id(self):
        for item in self:
            last_line = next((line for line in item.last_purchase_line_ids if line.order_id.type == 'Global'), False)
            item.last_purchase_line_id = last_line

    @api.depends("last_purchase_line_id")
    def _compute_last_purchase_line_id_info(self):
        for item in self:
            last_line = item.last_purchase_line_id
            if last_line and last_line.order_id.type == 'Global':
                item.last_purchase_price = last_line.price_unit
                item.last_purchase_date = last_line.date_order
                item.last_purchase_supplier_id = last_line.partner_id
                item.last_purchase_currency_id = last_line.currency_id
            else:
        ...
        for item in self:
            if item.show_last_purchase_price_currency:
                ...
                item.last_purchase_price_currency = rates.get(
                    item.last_purchase_currency_id.id, 1
                )
            else:
